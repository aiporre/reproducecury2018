...
int main (int argc, char *argv[])
{
    ...
    for(int i = 1; i < argc; ++i)
    {
        ...
        {
            std::cout << "allflow options:" << std::endl;
    ...
    if(!TypeFloat.compare("float"))
        return allflow<float>(ResFile.c_str(), DataFile.c_str(), PointsFileOut.c_str());
    else if(!TypeFloat.compare("double"))
        return allflow<double>(ResFile.c_str(), DataFile.c_str(), PointsFileOut.c_str());
