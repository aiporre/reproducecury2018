...
lgd{1} = ['LargeDef, ',num2str(sout{1}.elapsedTime),' s'];

s.useDef = 'LargeDef_InitParam';
s.useoptim = 'lbfgs_dlib';
s.optim_M = 10;
sout{2} = matchCpp(s,target);
lgd{2} = ['LargeDefInitParam (lbfgs\_dlib), ',num2str(sout{2}.elapsedTime),' s'];

s.useoptim = 'lbfgs';
sout{3} = matchCpp(s,target);
lgd{3} = ['LargeDefInitParam (liblbfgs), ',num2str(sout{3}.elapsedTime),' s'];

s.useoptim = 'lbfgs_Quentin';
sout{4} = matchCpp(s,target);
lgd{4} = ['LargeDefInitParam (lbfgs\_Quentin), ',num2str(sout{4}.elapsedTime),' s'];

% s.mom = sout{1}.mom(:,:,1);
% 
% sout{3} = matchCpp(s,target);
% lgd{3} = ['LargeDef puis LargeDefInitParam, ',num2str(sout{1}.elapsedTime+sout{3}.elapsedTime),' s'];
% 
% s.mom = sout{2}.mom;
% s.X = sout{2}.X;
% 
...
% 
% sout{4} = matchCpp(s,target);
% lgd{4} = ['LargeDefInitParam puis LargeDef, ',num2str(sout{2}.elapsedTime+sout{4}.elapsedTime),' s'];
% 
% s.mom = sout{4}.mom(:,:,1);
% s.useDef = 'LargeDef_InitParam';
% s.useoptim = 'lbfgs_dlib';
...
% lgd{5} = ['LargeDefInitParam puis LargeDef puis LargeDefInitParam, ',num2str(sout{2}.elapsedTime+sout{4}.elapsedTime+sout{5}.elapsedTime),' s'];
