...
showfaces = 0;                  % if 1 display faces of triangular mesh or edges of curve
facethickness = .5;
edgethickness = 2;
showpoints = 1;                 % if 1 display source and target points
...
gridcolor = .6*[1;1;1];         % color of grid lines
...
if ntargets
    ...
    case {'landmarks','measures'}
        showpoints = 1;
    case {'curvecurr','surfcurr','curvecycle'}
        showfaces = 1;
    end
end
...
if showtraj & length(size(X))==3
    ...
    if ntargets
        for k = 1:ntargets
            loadstruct('target{k}')
            ghandle.traj{k} = plot3(squeeze(X(1,target{k}.vx,1:stepflow))',squeeze(X(2,target{k}.vx,1:stepflow))',squeeze(X(3,target{k}.vx,1:stepflow))',trajtag);       % plot trajectories
            set(ghandle.traj{k},'LineWidth',trajwidth);
        end
    else
        ghandle.traj = plot3(squeeze(X(1,:,1:stepflow))',squeeze(X(2,:,1:stepflow))',squeeze(X(3,:,1:stepflow))',trajtag);
        set(ghandle.traj,'LineWidth',trajwidth);
    end
end
        ...
        if showfaces
            ...
            if dim >= 2
                ...
                if dim==3
                    ...
                    set(h.faces,'LineWidth',edgethickness);
                end
                legh = [legh,h.faces];
            end
        end
...
if size(X,2)==1
    ...
    sz = gridmargin * max(max(X,[],3),[],2)-min(min(X,[],3),[],2);
end
gridstep = sz/max(gridsize);
meanx = .5 * (max(max(X,[],3)',[],1)+min(min(X,[],3)',[],1));
g1 = (meanx(1)-sz(1)/2):gridstep:(meanx(1)+sz(1)/2);
s1 = length(g1);
g2 = (meanx(2)-sz(2)/2):gridstep:(meanx(2)+sz(2)/2);
s2 = length(g2);
if size(x,1)==3 && any(x(3,:))
    g3 = (meanx(3)-sz(3)/2):gridstep:(meanx(3)+sz(3)/2);
else
    g3 = 0;
end
s3 = length(g3);
...
g3 = g3(:)';
if size(x,1)==3
    g = flowCpp(s,[g1;g2;g3],1,stepflow);
else
    g = flowCpp(s,[g1;g2],1,stepflow);
    g(3,1) = 0;
end
