...
%               iteration; 'bar' displays a progression bar during minimization 
%               and the final output; 'final' displays just the final output; 'notify' 
...
if strcmp(optim.Display,'bar')
    wbar = waitbar(0,'estimated progression');
    estim_progress = 0;
end
...
while(true)
    ...
    if strcmp(optim.Display,'bar')
        estim_curr = max([data.iteration/optim.MaxIter,...
            log(max(abs(data.xOld-data.xInitial)))/log(optim.TolX),...
            log(gNorm)/log(optim.TolFun)]);
        estim_progress = max(estim_curr,.8*estim_progress+.2*estim_curr);
        waitbar(estim_progress,wbar);
    end
